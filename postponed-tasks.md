...
更新时间：2026-09-23（Asia/Singapore）

## 当前交付游标

- 当前唯一游标：在 `0e72060` 后续建机场 `passenger-boarding-bridge` 与写字楼 `office-elevator-car`，各150个可见物锚点、6个细节区，共+300锚点；将机场路径调整为 `boarding-gate → passenger-boarding-bridge → aircraft-cabin`，接待大厅新增可见电梯门入口，轿厢为终点场景。两场景各新增2段/12句双语会话。预期总量99场景、14,923锚点、642细节区、98门户；唯一展示词总数等待 GitHub `validate:data` 产物核实。锚点复核已把廊桥窗格移回可见窗框、门口标签移出入口热点，并校正电梯面板/门槛/地毯/顶棚位置。素材 JPEG SHA 分别为 `f2d8b7c4fde68f8c8abf1b05d708536bd956a64e892ffa620dd9732b80d52835` 与 `991fbda7b0bd3bf0527cb65600655fda73cbbf9ec3865aec0a75718a1593cd3b`；对应源 PNG SHA 为 `0c4fb4c93ddf746e6c7f9e9ab370ca6eda2c168c3bde2e3f4656b1c149548cba` 与 `1053e7aee909fff760d04a822796ce0613a68ba745e0f751afd50ca4ebc9b9f2`。生成器二次 `--check` 无差异。候选 `ff5e30a` 的 GitHub milestone 验收 run `35882640609` 在 `verify:full` 的 lint 阶段停止：新生成器中有一个未使用局部变量；已修复，待推送后重跑同一完整工作流。测试/build/E2E/性能门尚无结果；无完整通过证据前不部署。Site 已核实为公开，当前版本仍是 `https://hellowords-explore.lilifenghao44444.chatgpt.site/`。
- 接续状态：`c3b29b9` 修掉 lint 后，GitHub run `35883055780` 通过 lint 与 typecheck，在 `validate:data` 检出新场景缺少至少3个代表性排除词的 anchor-audit 证据，尚未进入单测/build/E2E/性能门。生成器现为两场景各记录8个明确排除项并重生成 JSON；二次 `--check` 与 `git diff --check` 通过。此修正待整批提交/推送并再次完整验收；线上仍为公开 v1，不部署至全绿。
