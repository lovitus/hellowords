...
- 公开源码仍冻结在 `8e3cab7039f684a434887faa76c6ba7c617ce8ea`；未发布的本地万词里程碑已推进到 `51483f5`（含此前的 `59840e3` 仓库与 `d363f52` 酒店批次）。当前累计 52 个场景、8,174 个空间锚点、7,090 个去重展示词、359 个细分区、51 个门户、6 条首页分支；距 10,000 个真实空间词还差 2,910。
- 当前未发布批次：新增 Office service core → Warehouse loading dock（121 词/5 区）、School campus → School library（121 词/5 区）、City street → Hotel exterior（69 词/6 区）→ Hotel lobby and rooms（107 词/5 区），以及首页 Market 面板 → Supermarket（115 词/5 区）。`npm run verify` 全绿；完整桌面/移动 E2E 122 通过、36 个设计性跳过；性能 2/2 通过。终端场景继续放大保持原场景，不进入万词大图；首页仍只在悬浮分区时显示该区词。
...
- 当前未发布里程碑为 7,090 个去重展示词，距 10,000 仍差 2,910。后续场景继续累积到同一里程碑，不生成逐场景公开版本号。
