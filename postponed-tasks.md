...
- 当前唯一游标：提交 `29e0a6f` 已推送到 `codex/practical-conversation-expansion`，新增终端场景 `histology-sectioning-workstation`，从病理实验室可见切片设备门户 `[1270,520,70,70]`、enterScale 3.35进入；子场景无门户，继续放大留在切片工作台。锚点终审通过：108个可见词、6区（19/18/16/17/19/19），从初稿移除12个重复、不可见或图像无法确认标签；所有坐标均在对应细节区内。另新增3组/18句双语会话。PNG SHA `e9c6440be9f18776133303b5e89269b74282f5e740f7d90e9b128e5bde4753cb`，1600×900 JPEG SHA `6d844355b38d760d56534ebd950bc2cff5f32e606f244005e5ffacf11a824292`。整世界现为102场景、15,481锚点、14,397全局唯一展示词、662细节区、101门户；10k内容目标已过。GitHub milestone preflight run `35920349511` 已取得终态失败（只读一次失败摘要）：正确性、构建、SSR及桌面/移动E2E通过，E2E为236通过/36设计性跳过；唯一失败为性能预算：转场p95 668.8ms（≤650）、浏览器CPU 770ms/次（≤750）、Renderer Task 125.9ms/次（≤75）、峰值PSS 354.5MiB（≤350）、保留PSS增长44.5MiB（≤40）；视觉转场、LCP、JS heap和DOM门槛通过。未降低预算、不发布；Site仍为公开v1。当前工作树已针对相机帧热路径减少重复区级揭词排序和碰撞去重分配，并增加与原实现等价的域测试；遵守要求，未在本机运行测试/build。下一步一次提交/推送该完整性能批次，运行GitHub milestone preflight；门禁通过后再按既有授权发布累积交付。医学设备名按 [Harvard/Leica 微切片机手册](https://arboretum.harvard.edu/wp-content/uploads/2020/07/leica_rm2155manual.pdf) 与 [Monash Histology 服务说明](https://www.monash.edu/researchinfrastructure/histology/our-services?external-uuid=933f4fa4-86f8-43e6-bea3-9954042f3ba0) 核对；会话不提供诊断或操作建议。

- 前一批急诊评估区的修订已在 `a4ccae2` 推送；GitHub run `35914132458` 的完整正确性、构建、SSR与桌面/移动 E2E通过，仅性能门失败：转场 p95 676ms（≤650）、浏览器 CPU 808.5ms/次（≤750）、Renderer TaskDuration 127.54ms/次（≤75）、峰值 PSS 362.4MiB（≤350）、保留 PSS 增量57.3MiB（≤40）；不降低预算。当前批次完成后才重新跑完整门禁；若性能仍失败则保留未发布状态，继续按用户优先级扩场景，不声称已达到发布验收。
- 上一个完整非里程碑 run `35904939872` 的数据/lint/typecheck/build/SSR/E2E通过，performance未通过：视觉转场 p95 271.3ms（≤650）、Playwright转场 p95 668.1ms（≤650失败）、浏览器CPU 790.5ms/次（≤750失败）、Renderer TaskDuration 130.589ms/次（≤75失败）、峰值PSS 352.499MiB（≤350失败）、PSS增长40.0068MiB（≤40失败）；JS heap、LCP和DOM通过。预算不放宽；新里程碑工作流需重新完整覆盖这些门槛后才能发布。
- Site 未发布；公开地址仍为 `https://hellowords-explore.lilifenghao44444.chatgpt.site/`。全部 GitHub 门槛通过后，再按一次批次授权发布一版。
- `0e72060` 的 GitHub run `35875508940` 已确认终态失败，失败日志确认 `Browser: success`、`Performance: failure`；220项浏览器测试通过、36项设计性跳过，场景完整性仍为97/14,623/13,539/630/96，构建和此前验证步骤完成。唯一失败是性能门：40次转场浏览器CPU 535.5ms/次（预算750，通过）、Renderer TaskDuration 98.04ms/次（预算75，失败）、峰值PSS 375.94MiB（预算350，失败）、保留PSS增长70.30MiB（预算40，失败）；视觉转场p95 266.3ms、冷启动LCP 232ms、JS heap峰值13.69MiB、live DOM 197通过。artifact 位于 `/Volumes/micron512g/tmp-project/codex-audit-tmp/hellowords-ci-35875508940/verification-0e72060f9c60603119242c90a724e69ef150927a/`。这证实功能对话及E2E没有阻断，剩余是反复出现的Renderer/PSS性能缺口；不放宽门槛、不为小数值反复微调。性能优化暂缓，继续按用户优先级增添实用场景和词汇；未通过完整门槛前不发布。当前公开站保持 https://hellowords-explore.lilifenghao44444.chatgpt.site/ 。
- 医院措辞依据 NHS ICU 探访与X-ray患者指引、FDA OTC药品标签说明；内容练习如何向医护人员求助、核对标签、说明过敏/语言需求、披露影像检查信息和请求解释，不提供诊断或剂量建议：[NHS ICU](https://www.nhs.uk/tests-and-treatments/intensive-care/)、[NHS X-ray](https://www.nhs.uk/tests-and-treatments/x-ray/)、[FDA Drug Facts label](https://www.fda.gov/drugs/understanding-over-counter-medicines/over-counter-drug-facts-label)。Artifact 位于上方外置审计目录；Site 未发布。
- 新批资产：`/scenes/airport-conveyor-drive-unit-premium-v1.jpg` SHA `084c09e4e7703bbd1fb116eefb4b2274e48a8cc1ae3fdbd7b4e0ecbdb42b7f23`，源 PNG SHA `dc0bfd0c79cf7c47e2c29b7237a7b9df2f0be031685aeb7aa185285fedbcb3f7`；`/scenes/office-network-rack-premium-v1.jpg` SHA `bc42d7c3cb274f47451d0bdbede8c90d2c6ae9f7b21256b99884e98d5fea19da`，源 PNG SHA `ac18cc16489c3ea82bdd5394fc4178fd3ad42a646e2a677778a1c4996e6d57f0`。机场输送部件沿用 MATREX 机场输送设备资料核对；交换机/配线架/机柜结构按 Cisco 机柜与物理基础设施资料核词。本批未做 Site 发布。

### 前序候选与历史验证记录

- 前序候选记录：`b6341a1` 的 GitHub run `35854025044` 通过 milestone 与 `validate:data`，342 个单测中 340 通过；修正 office 图片亮度合同及错位冰箱词锚点后，`d9fe061` 的 run `35856053324` 单测 342/343 通过，唯一失败为 `mobile-word/airport-baggage-conveyor` 首屏密度。随后已缩短 airport conveyor 词前缀；本次在其上新增两条终端设备细节路径，必须对最终候选重跑完整验收。
- `.github/workflows/ci.yml` 手动输入 `milestone_release_preflight` 会在 GitHub runner 运行 10k preflight 和同一干净提交的 `npm run verify:full`，不部署 Site；普通 CI 路径不变。首个候选 `17d3cb7` 的 GitHub run `35853384055` 在 `validate:data` 阶段失败（office-break-room 缺少 minLevel 4），随后已修正。
- 2026-09-23 继续新增机场行李输送带与写字楼休息区：各150个像素锚点、6个细节区；当前工作树预期为95个场景、14,323个锚点、13,239个全局唯一空间展示词、618个细节区、94个门户（以 GitHub correctness artifact 核实，纠正此前手算的612）。机场入口从 baggage-drop-station 进入；写字楼入口来自可见茶水间，并把 conference-room 门户从宽500收回到320像素，与新入口分开。两个新场景均为终点，继续放大不会跳入语义万词图。最终资产 `/scenes/airport-baggage-conveyor-premium-v1.jpg` SHA `d9447e581a12127dabcf7c9df53e091c4026aea517e12124610d9c0d53b8ed93`，源PNG SHA `8461dc2d0c350ec454fbd518589bace6ff2bef8918eda833df53a730892baa37`；`/scenes/office-break-room-premium-v1.jpg` SHA `1fe63b2f3ca69e6a33f655c583c01655118ad4400b8d0b95342f04ab75f0f53d`，源PNG SHA `87314df72314c000014d1c2320e5195adf824fad86f018125e57db7059b938e3`。图片由 ImageGen `photorealistic-natural` 初始模式生成，分别呈现可见行李输送带/滚筒/驱动机构与冰箱/咖啡设备/水槽/微波炉/桌椅/回收桶；不含人物、品牌或可读文字。机场术语参照 [MATREX 行李输送系统](https://www.matrex-airport.com/en/products-and-solutions/baggage-conveyor/) 与 [Vanderlande 机场输送系统](https://www.vanderlande.com/airports/systems/transportation/)；茶水间设施参照 [美国商务部办公室茶水间设备指南](https://www.commerce.gov/opog/space-allowance-and-management-program) 和 [NC State Office Kit of Parts](https://facilities.ofa.ncsu.edu/files/2025/10/Office-Kit-of-Parts.pdf)。继续沿用已安装 sharp 0.34.5，不加依赖。生成器已校验场景每区25词、源图/最终JPEG SHA及全局展示词无碰撞；GitHub milestone check 通过，`validate:data` 首次发现茶水间最高缩放档缺失，已把实际可见细节提升到 minLevel 4 并同步生成 JSON。后续 GitHub `verify:full` run `35854025044` 汇总 342 个单元测试中 340 通过、2 失败；E2E 与性能门未运行，不能发布。目视复核同时发现该最高档锚点落在冰箱搁架而非瓶盖，正在重新定锚。Site 未发布；公开地址仍为 https://hellowords-explore.lilifenghao44444.chatgpt.site/ 。
- `.github/workflows/ci.yml` 新增手动输入 `milestone_release_preflight`：勾选时在 GitHub runner 上运行 10k milestone preflight 和同一干净提交的 `npm run verify:full`，不部署 Site；普通 CI 路径不变。首个候选 `17d3cb7` 的 GitHub run `35853384055` 在 `validate:data` 阶段失败（office-break-room 缺少 minLevel 4）。修订后候选 `b6341a1` 的 run `35854025044` 通过 milestone 与 `validate:data`，在单元测试 340/342 阶段失败；等待修正锚点/测试后再提交同一交付批次并运行完整候选。只有全部通过才进入 Sites 发布。
- `.github/workflows/ci.yml` 新增手动输入 `milestone_release_preflight`：勾选时在 GitHub runner 上运行 10k milestone preflight 和同一干净提交的 `npm run verify:full`，不部署 Site；普通 CI 路径不变。首个候选 `17d3cb7` 的 GitHub run `35853384055` 在 `validate:data` 阶段失败（office-break-room 缺少 minLevel 4）；已修正同一批次，待推送修订后的同一交付提交，再运行一次完整发布候选工作流。只有全部通过才进入 Sites 发布。
- 本批目视复审后继续拆开了冰箱/柜体/急停按钮附近重合锚点，改正洗碗机踢脚板、灯带和砧板边缘，去除不可见“植物托盘”并补上下柜接缝与果蔬盒上沿；新终点场景的桌面最大缩放留在当前空间的 E2E 断言已加入。办公室休息区 detail-zone 仍会因吧台、橱柜与座位共享墙面而部分重叠；这些区生成的是焦点线索而非地图点击热点，暂不阻断本批，如实际操作显示线索难分再拆区。
- 续接新增终端场景 `school-locker-bank`：从学校走廊右侧可见储物柜进入，75 个图中锚点/5 个细分区；新增区域内词汇全局去重，门户不覆盖走廊已有锚点，也不与食堂入口相交。场景无子门户，连续放大不会跳入语义万词图。ImageGen 以 `photorealistic-natural` 生成并目视检查无人物/品牌/可读文字；复核时将长椅支腿锚点移到图像下沿可见支腿，并扩大对应焦点区。素材为1600×900 JPEG，SHA `8f03e0c4b267c589dce954be3191337945407f6df32cb6bb716610ba8209509c`，源PNG SHA `3b5c595f007d9aeffae0e9888dedeac9b3ea2a1e768ccb1ae551dd44645d1001`。工作树总计93场景、14,023锚点、12,939全局去重空间展示词、606细分区、92门户。
- `95c6b32` 的 workflow `35838301686` 曾因单元测试的大小写断言失败；数据中实际有 `Customs X-ray scanner`，修正已包含在 `8140fc2`。该修正候选 workflow `35838703808` 已确认终态失败：E2E 成功，只有性能门失败。40 次转场测得 all-Chrome CPU 935.5ms/次（预算750）、renderer TaskDuration 144.5ms/次（预算75）、峰值 PSS 382.6MiB（预算350）、PSS 保留增长75.2MiB（预算40）均超预算；视觉转场 p95 283.3ms、冷启动 LCP 348ms、JS heap 峰值14.36MiB 通过。未重跑或重启该终态 workflow；不能发布 Site。
- `e747d1b` 为储物柜新增2段/12句原创双语会话，并尝试在相机运动中取消避让过渡。GitHub workflow `35841889111` 已确认终态失败：全量浏览器 3 项未通过（连续缩放最大标签槽跳位157.3px，门槛120px；储物柜 E2E 第二句断言仍是改写前旧文本，在桌面/移动均失败），构建与正确性产物生成成功。性能：CPU 905.5ms/转场（预算750）、renderer TaskDuration 145.2ms/转场（预算75）、峰值 PSS 370.7MiB（预算350）、保留 PSS 增长72.3MiB（预算40）失败；视觉转场p95 271.8ms、LCP 340ms、JS heap峰值14.24MiB、live DOM 186通过。初次等待超时后，本轮用同一 handle 确认终态，失败日志助手调用一次但无正文；性能/E2E详情已下载到外置审计临时目录。公开 Site 仍为 v1。
- `0f70103` 恢复相机运动期间原有避让过渡并修正储物柜会话断言；GitHub workflow `35845525643` 的数据、lint/typecheck、单元、构建/SSR及完整浏览器（212通过、36个设计性跳过）通过，仅性能门失败。40 次转场 CPU 962.3ms（预算750）、renderer TaskDuration 162.4ms（预算75）、峰值PSS 387.9MiB（预算350）、保留PSS增长73.5MiB（预算40）超标；视觉转场p95 278.1ms、冷启动LCP 360ms、JS heap峰值13.61MiB、live DOM 186通过。该候选含稀疏摄像机布局试验，但没有降低本次测量值；产物保存在外置审计目录 `/Volumes/micron512g/tmp-project/codex-audit-tmp/hellowords-ci-35845525643.1LSJnu/`。未发布新 Site 版本。下一步回到场景/词汇扩展；性能门槛保持未通过，不得据此发布。

- 已提交并推送批次 `31e5289`：12 个场景、24 组会话、144 句双语练习。GitHub Actions 原运行 `35701128527` 已读取终态：E2E 198 通过、36 跳过；完整 workflow 因性能门槛失败。暖场景转场测得 all-Chrome CPU 976.5 ms/转场（上限750）、renderer TaskDuration 158.0 ms/转场（上限75）、峰值 PSS 450.7 MiB（上限350）、保留 PSS 增长98.9 MiB（上限40）；视觉转场 p95 为284.6 ms（上限650）。此前 `83dd12a` 同一渲染代码也测得945.8 ms、148.1 ms、463.1 MiB、100.7 MiB，性能缺口早于会话数据扩充。公开 Site 保持现有 v1。
- 前一候选 `958625d` 的 GitHub Actions 运行 `35825653637` 已确认终态失败：数据、构建、单元与全量浏览器 E2E 均通过，只有性能门未过。40 次切换测得 all-Chrome CPU 654ms/次（预算750，通过）、renderer TaskDuration 117.0ms/次（预算75，失败）、峰值 PSS 390.5MiB（预算350，失败）、保留 PSS 增长91.5MiB（预算40，失败）、JS heap峰值14.1MiB（预算96，通过）、视觉转场 p95 268.4ms（预算650，通过）；日志只读取一次，artifact 在忽略目录 `artifacts/ci-35825653637/verification-958625dd749fa7600873e9589055fb3b90ed3b5f/artifacts/perf/summary.json` 与 `samples.ndjson`。进程样本的增长主要在 renderer/GPU 进程，不能据此断言为 JS heap 泄漏。
- 当前批次扩充三条实用路线：急诊床位区、写字楼接待大厅、机场海关大厅各150个可见物锚点/6个细分区，共+450锚点；增加3个场景对话情境、6组、36句双语练习。合计91场景、13,798锚点、12,714全局去重展示词、595细分区、90门户；对话共27个场景情境/54组/324句（对话不计空间词）。最终JPEG已逐张查看，数据生成器报告全球显示词无碰撞且二次运行幂等。
- 候选 `db9547b` 的 workflow `35827695505` 已终态失败：全量 E2E 206通过/36跳过，构建报告通过；性能测量为 CPU 961.3ms/次、renderer task 157.1ms/次、峰值/保留 PSS 383.2/73.5MiB，均超预算；视觉转场 p95 269.1ms、LCP 356ms、JS heap峰值14.3MiB通过。产物在忽略目录 `artifacts/ci-35827695505/verification-db9547b7a417d9176ff79b24156d90fc51e0da1a/`。
- 当前已推送提交 `4455d54` 的 workflow `35829550570` 也已终态失败，失败日志确认 Performance 子门失败、Browser 成功。完整 E2E 206通过/36跳过；40次切换测得 CPU 711.3ms/次（预算750，通过）、renderer task 106.5ms/次（预算75，失败）、峰值 PSS 386.3MiB（预算350，失败）、保留 PSS 增长71.3MiB（预算40，失败）、视觉转场 p95 271.1ms（预算650，通过）、JS heap峰值13.7MiB（预算96，通过）。与前一分支 run 相比，CPU/task/样式计算下降，但这不是同次受控 A/B；稳定 ref 改法已让 CPU 门过线，渲染和 native memory 仍需继续。
- 本轮续建机场海关细分路线：从海关大厅可见的 X 光传送带进入 `customs-baggage-examination`，新增 150 个图中可见锚点/6 个细分区、2 组/12 句原创海关英语；与上个候选合计92场景、13,948锚点、12,864全局去重空间展示词、601细分区、91门户；会话为28情境/56组/336句（不计入空间词）。已生成并目视检查一张无人物/品牌/可读屏文的16:9主图；ImageGen 初始模式 `photorealistic-natural`，随后 `precise-object-edit` 只移除玻璃门伪文字纸张；PNG SHA `3756ff420c338f76d80300a106b74309d18f76bc203bf8ea464a007e4636f1e2`，最终1600×900 JPEG SHA `14415cb0597bdd93dcda2448a9c05b2aaf41e341a466a2f238ecdc0a9e611688`。生成器全局词去重通过；候选仍待完整回归验收。
- 候选 `d5969f5` 的 workflow `35833483029` 已终态失败。失败日志 helper 已按规只调用一次但未返回正文；该运行唯一上传产物 `artifacts/ci-35833483029/verification-d5969f5979537540adb3a2cdfcbd554cf3bd1692/artifacts/correctness/scene-integrity.json`，摘要确认92场景、13,948锚点、12,864去重展示词、601细分区、91门户，manifest 图完整。该包没有 build/E2E/perf 结果，因此确切失败步骤未核实；不重启终态运行。此后追加的储物柜场景尚待新候选 workflow 统一验证。
- 当前公开站：https://hellowords-explore.lilifenghao44444.chatgpt.site ，新站 v1，发布源码 `b202808596f869876ced1b87209bf6a6ec1a3412`（产品源码沿用已审核 `530f76c`）。公开版仍是88场景、13,348空间锚点、12,264去重展示词；工作树新增部分未发布。新域名人工业务冒烟尚未记录，不等于已验证。原站保留为历史记录。
- GitHub：`https://github.com/lovitus/hellowords`，用户已改为公开；`origin` 是 GitHub，`sites` 是新站，`sites-legacy` 是旧站。Sites 项目 ID `appgprj_6ab219c32ef0819198d4d7facbfd9cdc`。禁止重复创建站点或连续发布小版本。
- 会话参考 British Council A1/A2 情境表达和重复练习结构；使用现有 React 19 会话组件，不引入 H5P 等新框架或依赖。对话为原创短句，练习句数不计为空间锚点词汇。当前对话扩展不是10,000空间词目标的替代品。
- 性能状态：尚无当前候选通过证据，不能发布。`4455d54` 的性能文件显示进程增长主要在 renderer：PSS从约304MiB增至峰值386MiB，JS heap峰值仅13.7MiB；stable-ref 修正后 CPU已过线，但renderer task仍106.5ms/次且内存超限。后续优先检查高分辨率场景层与 compositor 资源保留，不降低锚点可读性、不放宽门槛。
- 已完成的代码修正：静态场景展示数据弱引用复用、已保留标签位置跳过无用备用排序、滚轮动画按真实帧间隔推进。正确性/构建已通过此前CI，但整体性能仍未达标；不能宣称流畅度问题已解决。无收益的滤镜移除已撤回。
- 本轮性能优化：`SceneViewport.applyCamera` 在 outgoing/incoming 的标签与交互层已被 CSS 完全隐藏时，不再逐帧计算碰撞布局/更新 portal 与 label DOM；通过稳定的 `transitionPhaseRef` 避免依赖变化重建相机回调，并在激活相位安排新帧。`4455d54` 全量 E2E通过，但性能仍未达到 renderer/PSS门槛。
- 验收方式：GitHub workflow 构建与测试，不使用私人runner；性能和浏览器测试均为发布硬门，两项都必须成功。最终产物上传可失败，但不能据此宣称有可发布产物。GitHub状态与失败日志只用用户指定全局工具，单个有界退避等待，禁止重复触发运行。
- 当前候选 `4455d54` 已推送；完整 E2E通过，性能门仍失败。下一步在同一万词现实探索目标下继续增加可准确抵达的密集场景/会话，并针对 renderer native-memory 资源保留做一处有证据的改进，再整批跑 GitHub workflow。公开 Site v1保持不变；透析室素材虽已审计，但医院大厅没有可明确指向透析室的可见入口，不伪造门户，保持延期。活动会话和无关临时文件未清理。

### 发布节制与万词里程碑（按用户后续授权更新）

- 禁止按单场景、小修复或 agent 批次连续公开版本。用户后续授权：新 Site 已公开，只有整批开发、审核与真实入口验证都完成后才发布一次；当前新 Site v1 保持不变，开发期间不调用 Sites 发布。
...
- 上一推送候选为12,714个全局去重空间展示词、13,798个锚点、91个现实场景；随后接入海关检查室和学校储物柜后工作树为12,939个展示词、14,023个锚点、93场景。计数已跨过10,000，但 `d5969f5` 的 workflow 失败且确切步骤未核实，新储物柜批次尚无 workflow 证据，不能发布。校园入口基于独立复核的可见区域；Hemodialysis unit（150词/6区）仍是已审计待接入素材。
