...
- 增加手动开启的同runner旧版对照：固定旧源码 `530f76ca4a1ce53e8ae7bf790bca3a618f40abd4`，仅复制同一性能测量脚本（产品源码不变），各自构建并顺序测量20轮，日志标明实际源码SHA。仅旧版诊断允许失败以继续测候选版，候选性能/E2E仍是硬门槛；默认CI不开启对照。本次在独立诊断分支手动运行一次，避免自动push重复执行；不发布、不改变阈值。
- 最新 `35698875012` / `547cb37`：正确性与构建通过，性能失败。CPU978.25ms、renderer157.170ms、PSS峰值451.069MiB/增量95.879MiB；相对上一轮同型号机器976/156.405的变化不足以证明速度收益，不能宣称计时修复解决了性能门。当前缺少同一Linux/Chromium环境下旧已审核源码的对照证据；下一步先对照旧基线，区分长期基线/运行环境差异与本批回归，停止继续凭总量试小修改。未发布，会话批次仍未完整验收。
