...
- 公开版本：`v51-849aa54`
- 地址：<https://hellowords-world.perky-spoon-0500.chatgpt.site/?v=51-849aa54>
- 源码：`849aa54`（v51 公开运行时；transit-hub 词汇扩展）
...
1. **高密度场景批次**：除 `world-map` 外，多数场景仍只有约 22–49 个经过审核的锚点。`apartment` 107、`city-street` 89、`transit-hub` 73、`kitchen` 116、`bedroom` 104、`bathroom` 103、科学馆 121、铁路站台 96、列车车厢 64、城市公园 63、叶片 49、聚合物 37、衣柜 56、植物细胞 46、叶绿体内部 51 已完成。接下来按 `docs/next-scene-batch.md` 继续逐场景制作明亮、可辨识的成品图，再从最终像素重新标注；不要用同义词、推断属性或重复部件填数。
...
- 本轮复核未发现新的 HelloWords/atlas 临时路径：`/private/tmp`、`/Volumes/micron512g/tmp-project` 和项目外置审计根中的同名路径均为空；v48、v49、v50 归档已由 Sites 保存，v51 发布压缩包在验证后移入系统废纸篓；共享临时根内其它项目任务继续保留。
