...
- 当前唯一游标：候选 `4a9d58a` 已补齐机场输送驱动单元与办公室网络机柜两个终端场景（共+300空间锚点）及各2段、6句双语对话；manifest 为97场景、14,623锚点、13,539个不重复英文展示词、630个细节区、96个门户。移动端机场首屏密度调整未移动锚点。原候选 `5bd8faf` 的 run `35869638454` 已确认域测试346/346、语义/词汇28/28、构建与浏览器 E2E通过，仅性能门失败。修订候选 `4a9d58a` 的 run `35872259645` 也已确认终态失败：场景完整性artifact为97场景/14,623锚点/13,539唯一展示词/630细节区/96门户，E2E为220通过/36设计性跳过，构建产物审计通过；性能唯一失败项显示浏览器CPU 760.75ms/转场（预算750）、Renderer TaskDuration 124.57ms（预算75）、峰值PSS 359.92MiB（预算350）、保留PSS增长45.995MiB（预算40），而视觉转场p95 269.4ms、冷启动LCP 356ms、JS heap峰值13.48MiB、live DOM 197通过。artifact位于 `/Volumes/micron512g/tmp-project/codex-audit-tmp/hellowords-ci-35872259645/verification-4a9d58a6783fd9606eafb03bcc63e283677dacf9/`。这是已知性能预算缺口，不放宽门槛，也不做无证据的大重构。
- 当前未提交医院对话扩充：给药房、急诊、住院床旁、ICU、手术室、放射科、病理科各增加1段六句会话，计划将总量从31场景/372句提升到31场景/414句；同步把住院床旁加入医疗练习免责声明，并更新急诊 E2E 行数预期。措辞参照 NHS 的 ICU 探访与 X-ray 患者指引、FDA 的 OTC 标签说明，仅练习如何求助、核对标签、披露相关信息和请求解释，不提供诊断/剂量建议：[NHS ICU](https://www.nhs.uk/tests-and-treatments/intensive-care/)、[NHS X-ray](https://www.nhs.uk/tests-and-treatments/x-ray/)、[FDA Drug Facts label](https://www.fda.gov/drugs/understanding-over-counter-medicines/over-counter-drug-facts-label)。改动尚未通过 GitHub 工作流；下一步先完成静态差异复核，再合为一个医院会话交付提交并跑单次完整 GitHub 验收。若性能仍失败则保留未发布候选，继续做实际内容开发；当前公开站保持 https://hellowords-explore.lilifenghao44444.chatgpt.site/ 。
