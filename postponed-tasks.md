...
- 公开源码仍冻结在 `8e3cab7039f684a434887faa76c6ba7c617ce8ea`；本地候选继续批量开发但不发布。当前已接入 80 个场景、12,143 个空间锚点、11,059 个去重展示词、529 个细分区、79 个门户、6 条首页分支；校园新增 School campus → Primary classroom（148 词/6 区）与 School campus → School gymnasium equipment（150 词/6 区），入口均来自校园父图中独立复核且互不重叠的完整房间区域。另有 150 词的 Hemodialysis unit 成品已完成资产与锚点审计，但医院父图没有可信可见入口，暂不伪造空间连接。
- 当前未发布批次在仓库、图书馆、酒店、超市基础上继续新增 Supermarket → Backroom（143 词/6 区）→ Walk-in cooler（144 词/6 区）、Airport → Security checkpoint（103 词/5 区）→ Boarding gate（120 词/6 区）→ Aircraft cabin（142 词/7 区）、Hospital → Emergency department（108 词/5 区）→ Operating theatre（109 词/5 区）→ Post-anesthesia care unit（134 词/6 区）。所有新词均通过全 manifest 去重；完整域测试 310、语义/词境测试 28 通过，桌面/移动 E2E 180 通过、36 个设计性跳过，性能 2/2 通过；另有 80 场景逐场桌面运行时遍历通过。终端场景继续放大保持原场景，不进入万词大图；首页仍只在悬浮分区时显示该区词。
...
- 当前已接入的未发布里程碑为 11,059 个去重展示词，已跨过 10,000；共 80 个现实场景、12,143 个空间锚点、79 条可进入路径。最近批次新增 School campus → Primary classroom 与 School campus → School gymnasium equipment；所有父入口均来自独立复核的清晰可见房间区域。Hemodialysis unit（150 词/6 区）保持为已审计待接入素材。仍冻结公开发布，后续继续整批扩充现实场景，由用户决定何时发布一个里程碑版本。
