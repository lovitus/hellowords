...
- 公开源码仍冻结在 `8e3cab7039f684a434887faa76c6ba7c617ce8ea`；未发布的本地万词里程碑已推进到 `1fe8fbc`。当前累计 57 个场景、8,757 个空间锚点、7,673 个去重展示词、386 个细分区、56 个门户、6 条首页分支；距 10,000 个真实空间词还差 2,327。
- 当前未发布批次在仓库、图书馆、酒店、超市基础上继续新增 Supermarket → Backroom（143 词/6 区）、Airport → Security checkpoint（103 词/5 区）→ Boarding gate（120 词/6 区）、Hospital → Emergency department（108 词/5 区）→ Operating theatre（109 词/5 区）。所有新词均通过全 manifest 去重；完整域/语义测试 262+28 通过，桌面/移动 E2E 134 通过、36 个设计性跳过，性能 2/2 通过。终端场景继续放大保持原场景，不进入万词大图；首页仍只在悬浮分区时显示该区词。
...
- 当前未发布里程碑为 7,673 个去重展示词，距 10,000 仍差 2,327。后续场景继续累积到同一里程碑，不生成逐场景公开版本号。
