...
- 当前未提交候选恢复连续相机运动时原有标签避让过渡（避免 `e747d1b` 实测157px槽位跳动），并修正储物柜 E2E 的旧中文断言。性能尝试只让摄像机热路径返回可见/交互布局，仍保留完整域 API 默认语义、选中词占位和已挂载近屏隐藏词几何；同时消除每帧中间数组/每锚点临时优先级对象。新增稀疏布局契约测试，仍未在本机测试/构建；仅 `git diff --check`，待整批 GitHub workflow 验证。
