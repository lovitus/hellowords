...
- 公开源码仍冻结在 `8e3cab7039f684a434887faa76c6ba7c617ce8ea`；本地候选继续批量开发但不发布。当前已接入 74 个场景、11,251 个空间锚点、10,167 个去重展示词、493 个细分区、73 个门户、6 条首页分支；另有 150 词的 Hemodialysis unit 成品已完成资产与锚点审计，但医院父图没有可信可见入口，暂不伪造空间连接。
...
- 当前已接入的未发布里程碑为 10,167 个去重展示词，已跨过 10,000；共 74 个现实场景、11,251 个空间锚点、73 条可进入路径。Aircraft cabin 已新增真实可见且与洗手间入口不重叠的 forward galley 分支（147 词/6 区）；Hemodialysis unit（150 词/6 区）保持为已审计待接入素材。仍冻结公开发布，完成整批回归与真实入口验收后再由用户决定是否发布一个里程碑版本。
