...
- 当前唯一游标：在 `0e72060` 后续建机场 `passenger-boarding-bridge` 与写字楼 `office-elevator-car`，各150个可见物锚点、6个细节区，共+300锚点；将机场路径调整为 `boarding-gate → passenger-boarding-bridge → aircraft-cabin`，接待大厅新增可见电梯门入口，轿厢为终点场景。两场景各新增2段/12句双语会话。预期总量99场景、14,923锚点、642细节区、98门户；唯一展示词总数等待 GitHub `validate:data` 产物核实。锚点复核已校正廊桥窗格/门口标签与电梯面板、门槛、地毯、顶棚位置。素材及生成器 SHA 详见本段后文。GitHub milestone runs `35882640609`、`35883055780`、`35883532358` 依次发现并修复 lint、anchor-audit 排除证据、重复 visual-region slug；最后一项是同词跨 zone 生成相同 region ID，现为 ID 添加稳定序号并将重复 `Elevator-car sill groove` 改为 `Elevator-car door-track slot`。生成器重跑与 `--check` 均通过，`git diff --check` 待本次更新后核验。下一步提交并推送完整候选，再跑同一 GitHub `milestone_release_preflight`；全量 build、单测、E2E 与性能门尚无通过证据，不能部署。公开 Site 仍为 v1：`https://hellowords-explore.lilifenghao44444.chatgpt.site/`。
