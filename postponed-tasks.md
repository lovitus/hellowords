...
- 该性能证据之后，本轮工作树追加了未提交、未验证的缩放过渡节流：拖动/动画缩放中直接跟随锚点并清除避让过渡，停止后只对大幅换位启动一次短过渡；`school-locker-bank` 新增2段/12句原创双语会话及相应域契约、真实校园路径 E2E。仅做 `git diff --check`；按仓库规则不在本机运行测试或构建。下一步先审阅该补丁，再整批运行 GitHub workflow；公开 Site 保持 v1。
