...
- 当前唯一游标：在 `0e72060` 后续建机场 `passenger-boarding-bridge` 与写字楼 `office-elevator-car`，各150个可见物锚点、6个细节区，共+300锚点；机场路径为 `boarding-gate → passenger-boarding-bridge → aircraft-cabin`，接待大厅新增可见电梯门入口，轿厢为终点场景；两场景各新增2段/12句双语会话。预期总量99场景、14,923锚点、642细节区、98门户；唯一展示词总数等待 GitHub `validate:data` 产物核实。锚点复核已校正廊桥窗格/门口标签与电梯面板、门槛、地毯、顶棚位置。素材及生成器 SHA 详见本段后文。GitHub runs `35882640609`、`35883055780`、`35883532358` 依次发现并修复 lint、anchor-audit 排除证据、重复 visual-region slug；提交 `eae70b5` 的 run `35884078499` 通过 lint、typecheck、词汇/语义/realm 校验，但 `validate:data` 在 `passenger-boarding-bridge` 报右扶手端盖锚点 (1120,486) 超出细节区右边界1110。现将该区宽度从720调为740以包住同一已复核可见扶手端盖；尚未提交/推送，生成器复核后应提交并重跑完整 `milestone_release_preflight`。单测、build、E2E、性能门尚无本候选通过证据，不能部署。公开 Site 仍为 v1：`https://hellowords-explore.lilifenghao44444.chatgpt.site/`。
