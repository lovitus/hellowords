...
- 当前公开站：https://hellowords-explore.lilifenghao44444.chatgpt.site ，新站 v1，发布源码 `b202808596f869876ced1b87209bf6a6ec1a3412`（产品源码沿用已审核 `530f76c`）。88 场景、13,348 空间锚点、12,264 去重展示词。新域名人工业务冒烟尚未记录，不等于已验证。原站保留为历史记录。
...
- 本批内容：场景会话已从 6 场景/12 组/72 句扩展为 12 场景/24 组/144 句。新增安检、机舱、行李提取、阅览室、会议室、学校后厨；整组阅读、逐句合成听读、遮译练习沿用现有实现，不增加框架或依赖。句子独立计数，不能计入空间词汇。参考 British Council 情境交流材料与 React 官方版本文档；原创会话，不复制教材。扩展内容尚未正式验收或发布。
- 性能状态：旧版对照步骤与候选性能步骤都失败，尚无全部通过证据。最近同机对照 `35699708037` / `83dd12a`：候选 CPU945.75ms/次、主线程148.085ms/次、PSS峰值463.113MiB，超过750/75/350门槛。旧版具体数值未回收，不能编造精确比较。40次切换 samples 显示GPU CPU20.81s、renderer16.4s，renderer PSS增加85.28MiB；大图渲染是下一诊断方向，不再凭总量猜测词索引优化。
- 已完成的代码修正：静态场景展示数据弱引用复用、已保留标签位置跳过无用备用排序、滚轮动画按真实帧间隔推进。正确性/构建已通过此前CI，但整体性能仍未达标；不能宣称流畅度问题已解决。无收益的滤镜移除已撤回。
- 验收方式：GitHub 单job构建与测试，不使用私人runner；性能和浏览器测试独立执行后由统一硬门共同判定，两项都必须成功。最终产物上传可失败，但不能据此宣称有可发布产物。账号产物存储目前已恢复，本地证据在忽略目录 `artifacts/ci-35699708037/`。GitHub状态与失败日志只用用户指定全局工具，单个有界退避等待，禁止重复触发运行。
- 唯一下一步：把会话扩展及独立验收编排作为一个批次提交并运行CI，取得内容/功能结果后继续大图性能定位；未满足完整门槛前不发布。学校冷库与医院透析仍延期：前者待素材，后者缺少可信父入口，不伪造连接。活动会话和无关临时文件未清理。
