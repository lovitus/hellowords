...
- 公开版本：`v93-<待部署>`
- 地址：<https://hellowords-world.perky-spoon-0500.chatgpt.site/?v=93-<待部署>>
- 源码：`待部署`（v93 本地验证中；首页维持仅悬浮分区显示锚点词，根图集与子场景小地图均按真实锚点包围盒聚焦，Apartment 与列车车厢/电动公交增密，连续放大避让平滑，Polymer 形貌增密，衣柜柜体/衣物部件增密，电池包硬件增密）
...
- v93 基于同一张 battery-premium-v2 原图补充 18 个清晰部件：cell can、module end plate、cable gland、terminal lug、coolant tube、lid port、lid recess、cell terminal stud、cell holder rib、module side wall、wire loom、tube retainer、coolant tee、front wall bolt、flange bolt、mounting tab、vent mesh、connector lock；电池包从 42 增至 60 个锚点，四个细节区分别为 14/17/16/13，原有锂离子电芯入口与锚点保持不变。`npm run verify`、全量 E2E 92 通过 / 36 个设计性跳过、性能 2/2 均通过；公开部署和入口冒烟待完成。
