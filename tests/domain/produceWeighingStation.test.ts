...
interface ProduceWeighingScene {
  readonly id: string;
...
test("the produce weighing station grounds 150 distinct visible station terms", async () => {
  const scene = await readJson<ProduceWeighingScene>("produce-weighing-station.json");
  assert.deepEqual(
    [scene.id, scene.parentId, scene.asset],
    ["produce-weighing-station", "supermarket-produce-department", "/scenes/produce-weighing-station-premium-v1.jpg"],
  );
  ...
  assert.deepEqual(scene.detailZones.map(({ id, labelIds }) => [id, labelIds.length]), [
    ["produce-weighing-station-zone-scale-hardware", 25],
    ["produce-weighing-station-zone-bag-roll-and-film", 25],
    ["produce-weighing-station-zone-counter-structure", 25],
    ["produce-weighing-station-zone-near-fruit-and-citrus", 25],
    ["produce-weighing-station-zone-greens-and-vegetable-edge", 25],
    ["produce-weighing-station-zone-lower-display-and-floor", 25],
  ]);
  assert.deepEqual([0, 1, 2, 3, 4].map((level) => scene.labels.filter(({ minLevel }) => minLevel === level).length), [25, 33, 32, 33, 27]);
  ...
  for (const file of (await readdir(sceneRoot)).filter((name) => name.endsWith(".json") && name !== "produce-weighing-station.json")) {
  ...
  const sourceBytes = await readFile(resolve(projectRoot, "scripts/assets/produce-weighing-station-v1.png"));
...
test("the produce weighing raster keeps a colorful, readable retail range", async () => {
  const metrics = await measure(resolve(projectRoot, "public/scenes/produce-weighing-station-premium-v1.jpg"));
  assert.ok(metrics.meanLuminance >= 88, `produce weighing mean luminance is ${metrics.meanLuminance.toFixed(2)}`);
  assert.ok(metrics.meanLuminance <= 145, `produce weighing mean luminance is ${metrics.meanLuminance.toFixed(2)}`);
  assert.ok(metrics.darkFraction <= 0.4, `produce weighing dark fraction is ${metrics.darkFraction.toFixed(4)}`);
  assert.ok(metrics.deepDarkFraction <= 0.19, `produce weighing deep-dark fraction is ${metrics.deepDarkFraction.toFixed(4)}`);
  assert.ok(metrics.meanChroma >= 0.18, `produce weighing mean chroma is ${metrics.meanChroma.toFixed(4)}`);
  assert.ok(metrics.meanChroma <= 0.3, `produce weighing mean chroma is ${metrics.meanChroma.toFixed(4)}`);
  assert.ok(metrics.channelMeanSpread <= 65, `produce weighing channel spread is ${metrics.channelMeanSpread.toFixed(2)}`);
});

test("the produce department exposes one bounded weighing-station entrance", async () => {
  const parent = await readJson<{
    ...
    readonly visualRegions: readonly Region[];
  }>("supermarket-produce-department.json");
  const portal = parent.portals.find(({ childSceneId }) => childSceneId === "produce-weighing-station");
  assert.ok(portal);
  assert.deepEqual([portal.x, portal.y, portal.width, portal.height], [459, 335, 431, 335]);
