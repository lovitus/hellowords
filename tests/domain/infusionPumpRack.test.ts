...
test("the infusion pump rack grounds 150 distinct visible equipment terms", async () => {
  const scene = await readJson<PumpRackScene>("infusion-pump-rack.json");
  assert.deepEqual(
    [scene.id, scene.parentId, scene.asset],
    ["infusion-pump-rack", "bedside-monitor-station", "/scenes/infusion-pump-rack-premium-v1.jpg"],
  );
  ...
  assert.deepEqual(scene.detailZones.map(({ id, labelIds }) => [id, labelIds.length]), [
    ["infusion-pump-rack-zone-upper-pump-modules", 25],
    ["infusion-pump-rack-zone-lower-pump-and-carrier", 25],
    ["infusion-pump-rack-zone-carrier-and-tubing", 25],
    ["infusion-pump-rack-zone-headwall-and-bedside", 25],
    ["infusion-pump-rack-zone-airway-and-suction", 25],
    ["infusion-pump-rack-zone-rear-bay-and-clinical-cart", 25],
  ]);
  assert.deepEqual([0, 1, 2, 3, 4].map((level) => scene.labels.filter(({ minLevel }) => minLevel === level).length), [32, 26, 47, 36, 9]);
  ...
  for (const file of (await readdir(sceneRoot)).filter((name) => name.endsWith(".json") && name !== "infusion-pump-rack.json")) {
  ...
  const sourceBytes = await readFile(resolve(projectRoot, "scripts/assets/infusion-pump-rack-v1.png"));
  assert.equal(createHash("sha256").update(sourceBytes).digest("hex"), "58e10b6cf62ecfd358804fb61d0f96be73f3e74e69a376e73e77e46834847f80");
...
test("the infusion-pump raster stays bright and readable for dense labels", async () => {
  const metrics = await measure(resolve(projectRoot, "public/scenes/infusion-pump-rack-premium-v1.jpg"));
  assert.ok(metrics.meanLuminance >= 130, `infusion pump mean luminance is ${metrics.meanLuminance.toFixed(2)}`);
  assert.ok(metrics.meanLuminance <= 180, `infusion pump mean luminance is ${metrics.meanLuminance.toFixed(2)}`);
  assert.ok(metrics.darkFraction <= 0.12, `infusion pump dark fraction is ${metrics.darkFraction.toFixed(4)}`);
  assert.ok(metrics.deepDarkFraction <= 0.05, `infusion pump deep-dark fraction is ${metrics.deepDarkFraction.toFixed(4)}`);
  assert.ok(metrics.meanChroma >= 0.05, `infusion pump mean chroma is ${metrics.meanChroma.toFixed(4)}`);
  assert.ok(metrics.meanChroma <= 0.16, `infusion pump mean chroma is ${metrics.meanChroma.toFixed(4)}`);
  assert.ok(metrics.channelMeanSpread <= 20, `infusion pump channel spread is ${metrics.channelMeanSpread.toFixed(2)}`);
});

test("the bedside monitor station exposes one bounded pump-rack entrance", async () => {
  const parent = await readJson<{
    ...
    readonly visualRegions: readonly Region[];
  }>("bedside-monitor-station.json");
  const portal = parent.portals.find(({ childSceneId }) => childSceneId === "infusion-pump-rack");
  assert.ok(portal);
  assert.deepEqual([portal.x, portal.y, portal.width, portal.height], [803, 124, 220, 296]);
