...
test("refrigerated display case keeps 141 reviewed cabinet and dairy terms", async () => {
  const scene = await readJson<Scene>("refrigerated-display-case.json");
  assert.equal(scene.id, "refrigerated-display-case");
  assert.equal(scene.parentId, "supermarket-grocery");
  assert.equal(scene.asset, "/scenes/refrigerated-display-case-premium-v1.jpg");
  assert.deepEqual([scene.width, scene.height], [1_600, 900]);
  assert.equal(scene.labels.length, 141);
  assert.equal(scene.visualRegions.length, 141);
  assert.deepEqual(scene.detailZones.map(({ id, labelIds }) => [id, labelIds.length]), [
    ["refrigerated-display-case-zone-cabinet-canopy", 20],
    ["refrigerated-display-case-zone-left-door-bank", 24],
    ["refrigerated-display-case-zone-center-door-bank", 25],
    ["refrigerated-display-case-zone-right-door-bank", 25],
    ["refrigerated-display-case-zone-shelf-product-detail", 25],
    ["refrigerated-display-case-zone-base-airflow", 22],
  ]);
  assert.deepEqual(
    [0, 1, 2, 3, 4].map((level) => scene.labels.filter(({ minLevel }) => minLevel === level).length),
    [12, 12, 38, 39, 40],
  );
  assert.equal(new Set(scene.labels.map(({ id }) => id)).size, 141);
  assert.equal(new Set(scene.labels.map(({ word }) => word.toLocaleLowerCase())).size, 141);
  ...
  assert.equal(scene.anchorAudit.reviewedAssetSha256, "80f2242e79d3edd8ac7bf10739a821c09a2c4f37e8abbc56e961f96be942b1a4");
  ...
  for (const word of new Set(scene.labels.map(({ word }) => word.toLocaleLowerCase()))) {
    assert.equal(existingWords.has(word), false, `display case does not duplicate ${word}`);
  }
  ...
  const sourceBytes = await readFile(resolve(projectRoot, "scripts/assets/refrigerated-display-case-v1.png"));
  assert.equal(
    createHash("sha256").update(sourceBytes).digest("hex"),
    "a70d630471004fd1ce297b9a94426ff691772ae73b99438ff80de21743d463f1",
  );
