...
test("city street v4 keeps 89 freshly grounded anchors and three destination contracts", async () => {
  ...
  assert.equal(scene.labels.length, 89);
  assert.deepEqual(
    [0, 1, 2, 3, 4].map((level) => scene.labels.filter((label) => label.minLevel === level).length),
    [8, 8, 11, 20, 42],
  );
  assert.equal(new Set(scene.labels.map(({ id }) => id)).size, 89);
  assert.equal(new Set(scene.labels.map(({ word }) => word.toLocaleLowerCase())).size, 89);
...
test("city street v4 anchors, regions, portals and six zoom zones stay on audited pixels", async () => {
  ...
  assert.equal(assigned.size, scene.labels.length, "all 89 anchors participate in local zoom exploration");
});
